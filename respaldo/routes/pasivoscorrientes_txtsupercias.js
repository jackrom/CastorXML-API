module.exports = app => {
     ...
     */
    app.get("/pasivoscorrientestxt", (req, res) => {
        PasivoscorrientesTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/pasivoscorrientestxt/:id", (req, res) => {
        PasivoscorrientesTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/pasivoscorrientestxt/:id", (req, res) => {
        PasivoscorrientesTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/pasivoscorrientestxt", (req, res) => {
        ...
        PasivoscorrientesTxt.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    PasivoscorrientesTxt.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    PasivoscorrientesTxt.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/pasivoscorrientestxt", (req, res) => {
        PasivoscorrientesTxt.update(req.body.cuentas, {
            where: {
