module.exports = app => {
     ...
     */
    app.get("/conciliacionanttxt", (req, res) => {
        ConciliacionAntTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/conciliacionanttxt/:id", (req, res) => {
        ConciliacionAntTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/conciliacionanttxt/:id", (req, res) => {
        ConciliacionAntTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/conciliacionanttxt", (req, res) => {
        ...
        ConciliacionAntTxt.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    ConciliacionAntTxt.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    ConciliacionAntTxt.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/conciliacionanttxt", (req, res) => {
        ConciliacionAntTxt.update(req.body.cuentas, {
            where: {
