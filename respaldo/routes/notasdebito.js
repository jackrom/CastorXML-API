...
module.exports = app => {
    const NotasDeDebito = app.db.models.Notasdedebito
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/notasdebito/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                NotasDeDebito.findById(
                    {
                        id: req.params.id
                    }
                )
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/notasdebito/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                NotasDeDebito.destroy({where: {id: req.params.id}})
                    .then(res => {
                        res.json(result)
                    })
                    .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/notasdebito", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                NotasDeDebito.create(req.body)
                    .then(result => {
                        res.json(result)
                    })
                    .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/notasdebito/savedocelectronico", (req, res) => {
        ...
        NotasDeDebito.findAll({
            ...
            .then(result => {
                if (result.data) {
                    ...
                    req.body.documento.direccionCliente = req.body.documento.direccioncliente
                    ...
                    req.body.documento.detalleList = JSON.stringify(req.body.documento.detalleList)
                    req.body.documento.impuestos = JSON.stringify(req.body.documento.impuestos)
                    req.body.documento.formasPagoList = JSON.stringify(req.body.documento.FormaPagoList)
                    console.log(req.body.documento)
                    NotasDeDebito.create(req.body.documento)
                        .then(result => {
                            let respuesta = {
                                error: false,
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/notasdebito", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                NotasDeDebito.update(req.query, {where: {id: req.query.id}})
                    .then(result => {
                        console.log(result)
                        res.json(result)
                        //res.sendStatus(204)
