...
module.exports = app => {
     ...
     */
    app.get("/puntosemision", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                PuntosEmision.findAll({
                    include: [{model: Empresas, as: 'empresamatriz'}],
                    order: [
                        ['id', 'DESC'],
                    ]
                })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/puntosemision/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                PuntosEmision.findById(
                    {
                        id: req.params.id
                    },
                    {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/puntosemision/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                PuntosEmision.destroy({where: {id: req.params.id}})
                    .then(res => {
                        res.json(result)
                    })
                    .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/puntosemision", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                PuntosEmision.create(req.body)
                    .then(result => {
                        res.json(result)
                    })
                    .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/puntosemision", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                PuntosEmision.update(req.query, {where: {id: req.query.id}})
                    .then(result => {
                        console.log(result)
                        res.json(result)
                        //res.sendStatus(204)
