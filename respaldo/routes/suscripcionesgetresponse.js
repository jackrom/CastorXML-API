module.exports = app => {
     ...
     */
    app.get("/suscripcionesgetresponsefc", (req, res) => {
        SuscripcionesGetResponseFC.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/suscripcionesgetresponsefc/:id", (req, res) => {
        SuscripcionesGetResponseFC.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/suscripcionesgetresponsefc/:id", (req, res) => {
        SuscripcionesGetResponseFC.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/suscripcionesgetresponsefc", (req, res) => {
        // console.log(req.body)
        SuscripcionesGetResponseFC.create(req.body)
            .then(result => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/suscripcionesgetresponsefc", (req, res) => {
        let datos = {
        ...
        SuscripcionesGetResponseFC.update(datos, {
            where: {
                id: req.body.obj.id
            }
        })
