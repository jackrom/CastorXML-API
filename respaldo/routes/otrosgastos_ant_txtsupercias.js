module.exports = app => {
     ...
     */
    app.get("/otrosgastosanttxt", (req, res) => {
        OtrosgastosAntTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/otrosgastosanttxt/:id", (req, res) => {
        OtrosgastosAntTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/otrosgastosanttxt/:id", (req, res) => {
        OtrosgastosAntTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/otrosgastosanttxt", (req, res) => {
        ...
        OtrosgastosAntTxt.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    OtrosgastosAntTxt.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    OtrosgastosAntTxt.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/otrosgastosanttxt", (req, res) => {
        OtrosgastosAntTxt.update(req.body.cuentas, {
            where: {
