module.exports = app => {
     ...
     */
    app.get("/pasivoscorrientesanttxt", (req, res) => {
        PasivoscorrientesAntTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/pasivoscorrientesanttxt/:id", (req, res) => {
        PasivoscorrientesAntTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/pasivoscorrientesanttxt/:id", (req, res) => {
        PasivoscorrientesAntTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/pasivoscorrientesanttxt", (req, res) => {
        ...
        PasivoscorrientesAntTxt.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    PasivoscorrientesAntTxt.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    PasivoscorrientesAntTxt.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/pasivoscorrientesanttxt", (req, res) => {
        PasivoscorrientesAntTxt.update(req.body.cuentas, {
            where: {
