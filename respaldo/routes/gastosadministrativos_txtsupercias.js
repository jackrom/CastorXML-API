module.exports = app => {
     ...
     */
    app.get("/gastosadministrativostxt", (req, res) => {
        GastosadministrativosTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/gastosadministrativostxt/:id", (req, res) => {
        GastosadministrativosTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/gastosadministrativostxt/:id", (req, res) => {
        GastosadministrativosTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/gastosadministrativostxt", (req, res) => {
        ...
        GastosadministrativosTxt.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    GastosadministrativosTxt.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    GastosadministrativosTxt.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/gastosadministrativostxt", (req, res) => {
        GastosadministrativosTxt.update(req.body.cuentas, {
            where: {
