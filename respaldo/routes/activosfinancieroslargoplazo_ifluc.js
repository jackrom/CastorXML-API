module.exports = app => {
     ...
     */
    app.get("/activosfinancieroslargoplazoifluc", (req, res) => {
        ActivosFinancierosLargoPlazoIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/activosfinancieroslargoplazoifluc/:id", (req, res) => {
        ActivosFinancierosLargoPlazoIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/activosfinancieroslargoplazoifluc/:id", (req, res) => {
        ActivosFinancierosLargoPlazoIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/activosfinancieroslargoplazoifluc", (req, res) => {
        ...
        ActivosFinancierosLargoPlazoIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    ActivosFinancierosLargoPlazoIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    ActivosFinancierosLargoPlazoIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/activosfinancieroslargoplazoifluc", (req, res) => {
        ActivosFinancierosLargoPlazoIfluc.update(req.body.cuentas, {
            where: {
