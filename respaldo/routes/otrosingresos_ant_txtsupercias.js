module.exports = app => {
     ...
     */
    app.get("/otrosingresosanttxt", (req, res) => {
        OtrosingresosAntTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/otrosingresosanttxt/:id", (req, res) => {
        OtrosingresosAntTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/otrosingresosanttxt/:id", (req, res) => {
        OtrosingresosAntTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/otrosingresosanttxt", (req, res) => {
        ...
        OtrosingresosAntTxt.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    OtrosingresosAntTxt.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    OtrosingresosAntTxt.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/otrosingresosanttxt", (req, res) => {
        OtrosingresosAntTxt.update(req.body.cuentas, {
            where: {
