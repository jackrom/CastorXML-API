module.exports = app => {
     ...
     */
    app.get("/userslocationfc", (req, res) => {
        Userslocationfc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/userslocationfc/:id", (req, res) => {
        Userslocationfc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/userslocationfc/:id", (req, res) => {
        Userslocationfc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/userslocationfc", (req, res) => {
        console.log('USER LOCATION REQUEST ', req.body.location)
        Userslocationfc.create(req.body.location)
            .then(result => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/userslocationfc", (req, res) => {
        console.log('UPDATE LOCATIONS REQUEST', req.body)
        Userslocationfc.update(req.body.obj, {
            where: {
