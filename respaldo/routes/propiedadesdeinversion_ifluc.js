module.exports = app => {
     ...
     */
    app.get("/propiedadesdeinversionifluc", (req, res) => {
        PropiedadesDeInversionIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/propiedadesdeinversionifluc/:id", (req, res) => {
        PropiedadesDeInversionIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/propiedadesdeinversionifluc/:id", (req, res) => {
        PropiedadesDeInversionIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/propiedadesdeinversionifluc", (req, res) => {
        ...
        PropiedadesDeInversionIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    PropiedadesDeInversionIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    PropiedadesDeInversionIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/propiedadesdeinversionifluc", (req, res) => {
        PropiedadesDeInversionIfluc.update(req.body.cuentas, {
            where: {
