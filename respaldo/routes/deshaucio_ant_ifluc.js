module.exports = app => {
     ...
     */
    app.get("/deshaucioantifluc", (req, res) => {
        DeshaucioAntIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/deshaucioantifluc/:id", (req, res) => {
        DeshaucioAntIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/deshaucioantifluc/:id", (req, res) => {
        DeshaucioAntIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/deshaucioantifluc", (req, res) => {
        ...
        DeshaucioAntIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    DeshaucioAntIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    DeshaucioAntIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/deshaucioantifluc", (req, res) => {
        DeshaucioAntIfluc.update(req.body.cuentas, {
            where: {
