...
module.exports = app => {
    const Detalles_ProductosFC = app.db.models.Detalles_productosfc
     ...
     */
    app.get("/detalles_productosfc", (req, res) => {
        Detalles_ProductosFC.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/detalles_productosfc/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Detalles_ProductosFC.destroy({
                    where: {
                        id: req.params.id
                    }
                })
    ...
    app.post("/detalles_productosfc", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                const ruta = {
                ...
                console.log(ruta)
                Detalles_ProductosFC.create(ruta)
                    .then(result => {
                        res.json(result)
                    })
                    .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/detalles_productosfc", (req, res) => {
        ...
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                ...
                Detalles_ProductosFC.update(ruta, {
                    where: {
                        id: req.body.rutaId
                    }})
                    .then(result => {
