...
module.exports = app => {
    const Retenciones = app.db.models.Retenciones
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/retenciones/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Retenciones.findById(
                    {
                        id: req.params.id
                    }
                )
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/retenciones/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Retenciones.destroy({where: {id: req.params.id}})
                    .then(res => {
                        res.json(result)
                    })
                    .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/retenciones", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Retenciones.create(req.body)
                    .then(result => {
                        res.json(result)
                    })
                    .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/retenciones/savedocelectronico", (req, res) => {
        ...
        Retenciones.findAll({
            ...
            .then(result => {
                if (result.data) {
                    ...
                    req.body.documento.impuestos = JSON.stringify(req.body.documento.detalleList)
                    console.log('retencionDB ', req.body.documento)
                    Retenciones.create(req.body.documento)
                        .then(result => {
                            let respuesta = {
                                error: false,
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/retenciones", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Retenciones.update(req.query, {where: {id: req.query.id}})
                    .then(result => {
                        console.log(result)
                        res.json(result)
                        //res.sendStatus(204)
