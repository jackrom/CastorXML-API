module.exports = app => {
    const EcpAntIfluc = app.db.models.EcpAntIfluc
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/ecpantIfluc/:id", (req, res) => {
        EcpAntIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/ecpantIfluc/:id", (req, res) => {
        EcpAntIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/ecpantIfluc", (req, res) => {
        EcpAntIfluc.update(req.body.cuentas, {
            where: {
