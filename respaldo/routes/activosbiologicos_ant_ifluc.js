module.exports = app => {
     ...
     */
    app.get("/activosbiologicosantifluc", (req, res) => {
        ActivosBiologicosAntIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/activosbiologicosantifluc/:id", (req, res) => {
        ActivosBiologicosAntIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/activosbiologicosantifluc/:id", (req, res) => {
        ActivosBiologicosAntIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/activosbiologicosantifluc", (req, res) => {
        ...
        ActivosBiologicosAntIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    ActivosBiologicosAntIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    ActivosBiologicosAntIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/activosbiologicosantifluc", (req, res) => {
        ActivosBiologicosAntIfluc.update(req.body.cuentas, {
            where: {
