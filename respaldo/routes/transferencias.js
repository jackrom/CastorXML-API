...
module.exports = app => {
    ...
    app.get("/transferencias", (req, res) => {
        Transferencias.findAll({
            include: [{model: Users, as: 'cliente'}],
            order: [
                ['id', 'DESC'],
            ]
        })
    ...
    app.get("/transferencias/:id", (req, res) => {
        Transferencias.findById(req.params.id)
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/transferencias/:id", (req, res) => {
        Transferencias.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
    ...
    app.post("/transferencias", (req, res) => {
        Transferencias.create(req.body)
            .then(result => {
                res.json(result)
            })
            .catch(error => {
    ...
    app.put("/transferencias", (req, res) => {
        Transferencias.update(req.query, {where: {id: req.query.id}})
            .then(result => {
                console.log(result)
                res.json(result)
                //res.sendStatus(204)
