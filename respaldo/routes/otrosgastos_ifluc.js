module.exports = app => {
     ...
     */
    app.get("/otrosgastosIfluc", (req, res) => {
        OtrosgastosIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/otrosgastosIfluc/:id", (req, res) => {
        OtrosgastosIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/otrosgastosIfluc/:id", (req, res) => {
        OtrosgastosIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/otrosgastosIfluc", (req, res) => {
        ...
        OtrosgastosIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    OtrosgastosIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    OtrosgastosIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/otrosgastosIfluc", (req, res) => {
        OtrosgastosIfluc.update(req.body.cuentas, {
            where: {
