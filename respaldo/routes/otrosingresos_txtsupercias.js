module.exports = app => {
     ...
     */
    app.get("/otrosingresostxt", (req, res) => {
        OtrosingresosTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/otrosingresostxt/:id", (req, res) => {
        OtrosingresosTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/otrosingresostxt/:id", (req, res) => {
        OtrosingresosTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/otrosingresostxt", (req, res) => {
        ...
        OtrosingresosTxt.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    OtrosingresosTxt.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    OtrosingresosTxt.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/otrosingresostxt", (req, res) => {
        OtrosingresosTxt.update(req.body.cuentas, {
            where: {
