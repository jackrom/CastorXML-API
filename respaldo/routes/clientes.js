...
module.exports = app => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/clientesByUser", (req, res) => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/clientesByUserByCompany/:empresaId", (req, res) => {
        console.log('ClientesByUserByCompany req.body')
        console.log(req.body)
        console.log('ClientesByUserByCompany req.params: ' + req.params.empresaId)
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Clientes.findAll({
                    where: {
