module.exports = app => {
     ...
     */
    app.get("/otrosgastosantIfluc", (req, res) => {
        OtrosgastosAntIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/otrosgastosantIfluc/:id", (req, res) => {
        OtrosgastosAntIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/otrosgastosantIfluc/:id", (req, res) => {
        OtrosgastosAntIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/otrosgastosantIfluc", (req, res) => {
        ...
        OtrosgastosAntIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    OtrosgastosAntIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    OtrosgastosAntIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/otrosgastosantIfluc", (req, res) => {
        OtrosgastosAntIfluc.update(req.body.cuentas, {
            where: {
