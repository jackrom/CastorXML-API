module.exports = app => {
     ...
     */
    app.get("/costosantIfluc", (req, res) => {
        CostosAntIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/costosantIfluc/:id", (req, res) => {
        CostosAntIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/costosantIfluc/:id", (req, res) => {
        CostosAntIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/costosantIfluc", (req, res) => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/costosantIfluc", (req, res) => {
        CostosAntIfluc.update(req.body.cuentas, {
            where: {
