module.exports = app => {
     ...
     */
    app.get("/activoscorrientesanttxt", (req, res) => {
        ActivoscorrientesAntTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/activoscorrientesanttxt/:id", (req, res) => {
        ActivoscorrientesAntTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/activoscorrientesanttxt/:id", (req, res) => {
        ActivoscorrientesAntTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/activoscorrientesanttxt", (req, res) => {
        ...
        ActivoscorrientesAntTxt.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    ActivoscorrientesAntTxt.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    ActivoscorrientesAntTxt.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/activoscorrientesanttxt", (req, res) => {
        ActivoscorrientesAntTxt.update(req.body.cuentas, {
            where: {
