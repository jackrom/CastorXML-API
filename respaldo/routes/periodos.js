module.exports = app => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/periodos/bycompany", (req, res) => {
        Periodos.findAll({
            where: {
    ...
    app.get("/periodos/byuser", (req, res) => {
        Periodos.findAll({
            include: [
                {
                    model: Empresas,
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/periodos/:id", (req, res) => {
        Periodos.findById(req.params.id)
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/periodos/:id", (req, res) => {
        Periodos.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
    ...
    app.post("/periodos", (req, res) => {
        ...
        Periodos.create(req.body.periodo)
            .then(result => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/periodos", (req, res) => {
        Periodos.update(req.query, {where: {id: req.query.id}})
            .then(result => {
                console.log(result)
                res.json(result)
                //res.sendStatus(204)
