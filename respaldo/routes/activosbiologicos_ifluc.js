module.exports = app => {
     ...
     */
    app.get("/activosbiologicosifluc", (req, res) => {
        ActivosBiologicosIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/activosbiologicosifluc/:id", (req, res) => {
        ActivosBiologicosIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/activosbiologicosifluc/:id", (req, res) => {
        ActivosBiologicosIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/activosbiologicosifluc", (req, res) => {
        ...
        ActivosBiologicosIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    ActivosBiologicosIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    ActivosBiologicosIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/activosbiologicosifluc", (req, res) => {
        ActivosBiologicosIfluc.update(req.body.cuentas, {
            where: {
