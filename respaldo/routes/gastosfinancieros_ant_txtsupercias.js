module.exports = app => {
     ...
     */
    app.get("/gastosfinancierosanttxt", (req, res) => {
        GastosfinancierosAntTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/gastosfinancierosanttxt/:id", (req, res) => {
        GastosfinancierosAntTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/gastosfinancierosanttxt/:id", (req, res) => {
        GastosfinancierosAntTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/gastosfinancierosanttxt", (req, res) => {
        ...
        GastosfinancierosAntTxt.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    GastosfinancierosAntTxt.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    GastosfinancierosAntTxt.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/gastosfinancierosanttxt", (req, res) => {
        GastosfinancierosAntTxt.update(req.body.cuentas, {
            where: {
