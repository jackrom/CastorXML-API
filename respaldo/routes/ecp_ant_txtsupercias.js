module.exports = app => {
    const EcpAntTxt = app.db.models.EcpAntTxt
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/ecpanttxt/:id", (req, res) => {
        EcpAntTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/ecpanttxt/:id", (req, res) => {
        EcpAntTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/ecpanttxt", (req, res) => {
        EcpAntTxt.update(req.body.cuentas, {
            where: {
