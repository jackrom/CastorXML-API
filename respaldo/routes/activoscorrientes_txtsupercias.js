module.exports = app => {
     ...
     */
    app.get("/activoscorrientestxt", (req, res) => {
        ActivoscorrientesTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/activoscorrientestxt/:id", (req, res) => {
        ActivoscorrientesTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/activoscorrientestxt/:id", (req, res) => {
        ActivoscorrientesTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/activoscorrientestxt", (req, res) => {
        ...
        ActivoscorrientesTxt.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    ActivoscorrientesTxt.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    ActivoscorrientesTxt.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/activoscorrientestxt", (req, res) => {
        ActivoscorrientesTxt.update(req.body.cuentas, {
            where: {
