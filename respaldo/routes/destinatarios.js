...
module.exports = app => {
    const Destinatarios = app.db.models.Destinatarios
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/destinatariosByUser", (req, res) => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/destinatariosByUserByCompany/:empresaId", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Destinatarios.findAll({
                    where: {
    ...
    app.post("/destinatarios", (req, res) => {
        ...
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                ...
                Destinatarios.create(destinatario)
                    .then(result => {
                        res.json(result)
                    })
                    .catch(error => {
    ...
    app.put("/destinatarios", (req, res) => {
        ...
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                ...
                Destinatarios.update(destinatario, {
                    where: {
                        id: req.body.ruc
                    }})
                    .then(result => {
