module.exports = app => {
     ...
     */
    app.get("/impuestosdiferidosifluc", (req, res) => {
        ImpuestosDiferidosIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/impuestosdiferidosifluc/:id", (req, res) => {
        ImpuestosDiferidosIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/impuestosdiferidosifluc/:id", (req, res) => {
        ImpuestosDiferidosIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/impuestosdiferidosifluc", (req, res) => {
        ...
        ImpuestosDiferidosIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    ImpuestosDiferidosIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    ImpuestosDiferidosIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/impuestosdiferidosifluc", (req, res) => {
        ImpuestosDiferidosIfluc.update(req.body.cuentas, {
            where: {
