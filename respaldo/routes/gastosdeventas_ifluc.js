module.exports = app => {
     ...
     */
    app.get("/gastosdeventasIfluc", (req, res) => {
        GastosdeventasIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/gastosdeventasIfluc/:id", (req, res) => {
        GastosdeventasIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/gastosdeventasIfluc/:id", (req, res) => {
        GastosdeventasIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/gastosdeventasIfluc", (req, res) => {
        ...
        GastosdeventasIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    GastosdeventasIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    GastosdeventasIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/gastosdeventasIfluc", (req, res) => {
        GastosdeventasIfluc.update(req.body.cuentas, {
            where: {
