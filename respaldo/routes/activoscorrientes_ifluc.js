module.exports = app => {
     ...
     */
    app.get("/activoscorrientesIfluc", (req, res) => {
        ActivoscorrientesIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/activoscorrientesIfluc/:id", (req, res) => {
        ActivoscorrientesIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/activoscorrientesIfluc/:id", (req, res) => {
        ActivoscorrientesIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/activoscorrientesIfluc", (req, res) => {
        ...
        ActivoscorrientesIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    ActivoscorrientesIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    ActivoscorrientesIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/activoscorrientesIfluc", (req, res) => {
        ActivoscorrientesIfluc.update(req.body.cuentas, {
            where: {
