module.exports = app => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/socialnetworkstouserfc/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                SocialnetworksToUserfc.findOne({where: {idNet: req.params.id}})
                    .then(result => res.json(result))
                    .catch(error => {
                        res.status(412).json({msg: error.message});
                    });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/socialnetworkstouserfc/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                SocialnetworksToUserfc.destroy({where: {idNet: req.params.id}})
                    .then(res => {
                        res.json(result)
                    })
                    .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/socialnetworkstouserfc", (req, res) => {
        // console.log(req.body)
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                SocialnetworksToUserfc.create(req.body)
                    .then(result => {
                        res.json(result)
                    })
                    .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/socialnetworkstouserfc", (req, res) => {
        // console.log(req.body)
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                let datos = {
                ...
                SocialnetworksToUserfc.update(datos, {
                    where: {
