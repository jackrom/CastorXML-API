module.exports = app => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/otrosifluc/:id", (req, res) => {
        OtrosIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/otrosifluc/:id", (req, res) => {
        OtrosIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/otrosifluc", (req, res) => {
        OtrosIfluc.update(req.body.cuentas, {
            where: {
