...
module.exports = app => {
    const Transportistas = app.db.models.Transportistas
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/transportistasByUser/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Transportistas.findAll({
                    where: {
    ...
    app.delete("/transportistas/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Transportistas.destroy({where: {id: req.params.id}})
                    .then(res => {
                        res.json(result)
                    })
                    .catch(error => {
    ...
    app.post("/transportistas", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Transportistas.create(req.body)
                    .then(result => {
                        res.json(result)
                    })
                    .catch(error => {
    ...
    app.put("/transportistas", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                ...
                const datos = {
                    identificacion_transportista: req.body.identificacion_transportista,
                    razon_social_transportista: req.body. razon_social_transportista,
                    correo_transportista: req.body.correo_transportista,
                    placa: req.body.placa,
                    empresaId: req.body.empresaId
                }
                Transportistas.update(datos, {
                    where: {
                        id: req.body.transportistaId
                    }
                })
                .then(result => {
                    console.log(result)
