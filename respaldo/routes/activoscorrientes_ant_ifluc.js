module.exports = app => {
     ...
     */
    app.get("/activoscorrientesantIfluc", (req, res) => {
        ActivoscorrientesAntIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/activoscorrientesantIfluc/:id", (req, res) => {
        ActivoscorrientesAntIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/activoscorrientesantIfluc/:id", (req, res) => {
        ActivoscorrientesAntIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/activoscorrientesantIfluc", (req, res) => {
        ...
        ActivoscorrientesAntIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    ActivoscorrientesAntIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    ActivoscorrientesAntIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/activoscorrientesantIfluc", (req, res) => {
        ActivoscorrientesAntIfluc.update(req.body.cuentas, {
            where: {
