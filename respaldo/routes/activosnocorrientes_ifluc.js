module.exports = app => {
     ...
     */
    app.get("/activosnocorrientesIfluc", (req, res) => {
        ActivosnocorrientesIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/activosnocorrientesIfluc/:id", (req, res) => {
        ActivosnocorrientesIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/activosnocorrientesIfluc/:id", (req, res) => {
        ActivosnocorrientesIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/activosnocorrientesIfluc", (req, res) => {
        ...
        ActivosnocorrientesIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    ActivosnocorrientesIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    ActivosnocorrientesIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/activosnocorrientesIfluc", (req, res) => {
        ActivosnocorrientesIfluc.update(req.body.cuentas, {
            where: {
