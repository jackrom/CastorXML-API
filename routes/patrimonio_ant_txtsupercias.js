module.exports = app => {
     ...
     */
    app.get("/patrimonioanttxt", (req, res) => {
        PatrimonioAntTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/patrimonioanttxt/:id", (req, res) => {
        PatrimonioAntTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/patrimonioanttxt/:id", (req, res) => {
        PatrimonioAntTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/patrimonioanttxt", (req, res) => {
        ...
        PatrimonioAntTxt.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    PatrimonioAntTxt.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    PatrimonioAntTxt.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/patrimonioanttxt", (req, res) => {
        PatrimonioAntTxt.update(req.body.cuentas, {
            where: {
