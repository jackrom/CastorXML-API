module.exports = app => {
     ...
     */
    app.get("/conciliacionIfluc", (req, res) => {
        ConciliacionIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/conciliacionIfluc/:id", (req, res) => {
        ConciliacionIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/conciliacionIfluc/:id", (req, res) => {
        ConciliacionIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/conciliacionIfluc", (req, res) => {
        ...
        ConciliacionIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    ConciliacionIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    ConciliacionIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/conciliacionIfluc", (req, res) => {
        ConciliacionIfluc.update(req.body.cuentas, {
            where: {
