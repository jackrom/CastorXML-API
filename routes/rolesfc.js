module.exports = app => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/rolesfc/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Rolesfc.findOne({where: {id: req.params.id}})
                    .then(result => res.json(result))
                    .catch(error => {
                        res.status(412).json({msg: error.message});
                    });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/rolesfc/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Rolesfc.destroy({where: {id: req.params.id}})
                    .then(res => {
                        res.json(result)
                    })
                    .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/rolesfc", (req, res) => {
        // console.log(req.body)
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Rolesfc.create(req.body)
                    .then(result => {
                        res.json(result)
                    })
                    .catch(error => {
