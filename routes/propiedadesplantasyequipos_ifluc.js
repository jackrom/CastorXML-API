module.exports = app => {
     ...
     */
    app.get("/propiedadesplantasyequiposifluc", (req, res) => {
        PropiedadesPlantasyEquiposIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/propiedadesplantasyequiposifluc/:id", (req, res) => {
        PropiedadesPlantasyEquiposIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/propiedadesplantasyequiposifluc/:id", (req, res) => {
        PropiedadesPlantasyEquiposIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/propiedadesplantasyequiposifluc", (req, res) => {
        ...
        PropiedadesPlantasyEquiposIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    PropiedadesPlantasyEquiposIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    PropiedadesPlantasyEquiposIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/propiedadesplantasyequiposifluc", (req, res) => {
        PropiedadesPlantasyEquiposIfluc.update(req.body.cuentas, {
            where: {
