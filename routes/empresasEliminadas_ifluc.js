module.exports = app => {
    const EmpresasEliminadasIfluc = app.db.models.EmpresasEliminadasIfluc
     ...
     */
    app.get("/empresaseliminadasIfluc", (req, res) => {
        EmpresasEliminadasIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/empresaseliminadasIfluc/byUser", (req, res) => {
        EmpresasEliminadasIfluc.findAll({
            where: {
            ...
            .then(results => {
                ...
                const totalEmpresasEliminadasIfluc = filteredEmpresas.length
                ...
                res.json({ empresas: filteredEmpresas, totalPage, totalEmpresasEliminadasIfluc})
            })
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/empresaseliminadasIfluc/:id", (req, res) => {
        EmpresasEliminadasIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/empresaseliminadasIfluc/:id", (req, res) => {
        EmpresasEliminadasIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/empresaseliminadasIfluc", (req, res) => {
        console.log("BODY: ", req.body)
        console.log("QUERY: ", req.query)
        const data = req.body.dataEmpresa
        EmpresasIfluc.findAll({
            ...
            .then(resultados => {
                const empresa = {
                    ...
                    userId: data.userId,
                    fechaCreacion: resultados[0].createdAt,
                }
                console.log("empresa: ", empresa)
                if (resultados && resultados.length > 0) {
                    EmpresasEliminadasIfluc.create(empresa)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/empresaseliminadasIfluc", (req, res) => {
        EmpresasEliminadasIfluc.update(req.body.cuentas, {
            where: {
