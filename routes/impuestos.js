...
module.exports = app => {
    const Impuestos = app.db.models.Impuestos
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/impuestosbycodigo/:codigo", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Impuestos.findById(
                    {
    ...
    app.delete("/impuestos/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Impuestos.destroy({where: {id: req.params.id}})
                    .then(res => {
                        res.json(result)
                    })
                    .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/impuestos", (req, res) => {
        console.log(req.body)
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Impuestos.update(req.body, {where: {id: req.body.uid}})
                    .then(result => {
                        console.log(result)
                        res.json(result)
                        //res.sendStatus(204)
