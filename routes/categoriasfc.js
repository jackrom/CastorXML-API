...
module.exports = app => {
     ...
     */
    app.get("/categoriasfc", (req, res) => {
        CategoriasFC.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/categoriasfc/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                CategoriasFC.destroy({
                    where: {
                        id: req.params.id
                    }
                })
