module.exports = app => {
     ...
     */
    app.get("/conciliaciontxt", (req, res) => {
        ConciliacionTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/conciliaciontxt/:id", (req, res) => {
        ConciliacionTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/conciliaciontxt/:id", (req, res) => {
        ConciliacionTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/conciliaciontxt", (req, res) => {
        ...
        ConciliacionTxt.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    ConciliacionTxt.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    ConciliacionTxt.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/conciliaciontxt", (req, res) => {
        ConciliacionTxt.update(req.body.cuentas, {
            where: {
