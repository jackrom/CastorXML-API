module.exports = app => {
     ...
     */
    app.get("/pasivosnocorrientestxt", (req, res) => {
        PasivosnocorrientesTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/pasivosnocorrientestxt/:id", (req, res) => {
        PasivosnocorrientesTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/pasivosnocorrientestxt/:id", (req, res) => {
        PasivosnocorrientesTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/pasivosnocorrientestxt", (req, res) => {
        ...
        PasivosnocorrientesTxt.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    PasivosnocorrientesTxt.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    PasivosnocorrientesTxt.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/pasivosnocorrientestxt", (req, res) => {
        PasivosnocorrientesTxt.update(req.body.cuentas, {
            where: {
