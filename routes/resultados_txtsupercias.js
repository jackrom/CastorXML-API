module.exports = app => {
     ...
     */
    app.get("/resultadostxt", (req, res) => {
        ResultadosTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/resultadostxt/:id", (req, res) => {
        ResultadosTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/resultadostxt/:id", (req, res) => {
        ResultadosTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/resultadostxt", (req, res) => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/resultadostxt", (req, res) => {
        ResultadosTxt.update(req.body.cuentas, {
            where: {
