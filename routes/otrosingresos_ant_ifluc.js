module.exports = app => {
     ...
     */
    app.get("/otrosingresosantIfluc", (req, res) => {
        OtrosingresosAntIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/otrosingresosantIfluc/:id", (req, res) => {
        OtrosingresosAntIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/otrosingresosantIfluc/:id", (req, res) => {
        OtrosingresosAntIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/otrosingresosantIfluc", (req, res) => {
        ...
        OtrosingresosAntIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    OtrosingresosAntIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    OtrosingresosAntIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/otrosingresosantIfluc", (req, res) => {
        OtrosingresosAntIfluc.update(req.body.cuentas, {
            where: {
