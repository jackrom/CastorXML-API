...
module.exports = app => {
    const Datafast = app.db.models.Datafast
    const Users = app.db.models.Users
    const RecurrentesdatafastFC = app.db.models.Recurrentesdatafastfc
    ...
    app.get("/datafast", (req, res) => {
        Datafast.findAll({
            ...
            .then(result => {
                result.forEach(obj => {
                    let contenido = (obj.cart && typeof obj.cart === 'string') ? JSON.parse(obj.cart) : null
                    obj.cart = (contenido && contenido.items) ? contenido.items[0].name : null
                    let cliente = JSON.parse(obj.customer)
                    obj.customer = (cliente) ? cliente.givenName + ' ' + cliente.middleName : 'No Disponible'
                    obj.result = JSON.parse(obj.result).code
                })
                res.json(result)
            })
            .catch(error => {
    ...
    app.get("/datafast/recurrentes", (req, res) => {
        Datafast.findAll({
            where: {
            ...
            .then(result => {
                result.forEach(obj => {
                    let contenido = JSON.parse(obj.contenidoCompra)[0]
                    obj.contenidoCompra = {
                        nombre: (contenido.infoproducto) ? contenido.infoproducto.nombre : contenido.nombre,
                        pagos: contenido.cantidadpagos
                    }
                    let cliente = JSON.parse(obj.customer)
                    obj.customer = {
                        nombre: cliente.givenName + ' ' + cliente.middleName,
                        identificationDocId: cliente.identificationDocId
                    }
                })
                res.json(result)
            })
    ...
    app.get("/reparartablarecurrentes", (req, res) => {
        Datafast.findAll({
            where: {
                recurringType: 'REPEATED',
                registrationId: {
            ...
            .then(result => {
                result.forEach(obj => {
                    let contenido = JSON.parse(obj.contenidoCompra)[0]
                    let dc = new Date(obj.fecha)

                    let recurrente = {
                        idDatafast: obj.id,
                        idCliente: obj.idCliente,
                        amount: obj.amount,
                        paymentBrand: obj.paymentBrand,
                        responseId: obj.responseId,
                        registrationId: obj.registrationId,
                        contenidoCompra: (contenido.infoproducto) ? contenido.infoproducto.nombre : contenido.nombre,
                        recurrenciaactiva: obj.recurrenciaactiva,
                        fecha: obj.fecha,
                        diacobro: dc.getDate()
                    }

                    RecurrentesdatafastFC.create(recurrente)
                })
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/datafast/:id", (req, res) => {
        Datafast.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/datafast", (req, res) => {
        Datafast.create(req.body.transaccion)
            .then(result => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/datafast", (req, res) => {
        Datafast.update(req.body.transaccion, {
            where: {id: req.body.transaccion.identificacion}
        })
            .then(result => {
                console.log(result)
                res.json(result)
