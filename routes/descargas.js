...
module.exports = app => {
    const Descargas = app.db.models.Descargas
            ...
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Descargas.findAll({
                    where: {
                        ...
                        mesNumero: req.body.mes,
                        anioNumero: req.body.anio
                    },
                    order: [
                        ['id', 'DESC'],
                    ]
    ...
    app.get("/descargasAcumuladasMensuales", (req, res) => {
        Descargas.findAll({
            attributes: [
                [Sequelize.literal(`COUNT(documentos)`), 'vecesdescargas'],
                [Sequelize.literal(`mesNumero`), 'mesNumero'],
                [Sequelize.literal(`anioNumero`), 'anioNumero'],
                [Sequelize.literal(`mes`), 'mes'],
                [Sequelize.literal(`userId`), 'user'],
                [Sequelize.literal(`SUM(documentos)`), 'count']
            ],
            group: [
                'user','mesNumero'
            ]
    ...
    app.post("/descargas", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                const load = {
                    userId: req.body.descarga.userId,
                    documentos: req.body.descarga.documentos,
                    mes: req.body.descarga.mes,
                    mesNumero: req.body.descarga.mesNumero,
                    anioNumero: req.body.descarga.anioNumero
                }
                Descargas.create(load)
                    .then(result => {
                        res.json(result)
                    })
                    .catch(error => {
