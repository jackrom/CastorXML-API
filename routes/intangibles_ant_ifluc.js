module.exports = app => {
     ...
     */
    app.get("/intangiblesantifluc", (req, res) => {
        IntangiblesAntIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/intangiblesantifluc/:id", (req, res) => {
        IntangiblesAntIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/intangiblesantifluc/:id", (req, res) => {
        IntangiblesAntIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/intangiblesantifluc", (req, res) => {
        ...
        IntangiblesAntIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    IntangiblesAntIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    IntangiblesAntIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/intangiblesantifluc", (req, res) => {
        IntangiblesAntIfluc.update(req.body.cuentas, {
            where: {
