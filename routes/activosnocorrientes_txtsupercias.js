module.exports = app => {
     ...
     */
    app.get("/activosnocorrientestxt", (req, res) => {
        ActivosnocorrientesTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/activosnocorrientestxt/:id", (req, res) => {
        ActivosnocorrientesTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/activosnocorrientestxt/:id", (req, res) => {
        ActivosnocorrientesTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/activosnocorrientestxt", (req, res) => {
        ...
        ActivosnocorrientesTxt.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    ActivosnocorrientesTxt.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    ActivosnocorrientesTxt.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/activosnocorrientestxt", (req, res) => {
        ActivosnocorrientesTxt.update(req.body.cuentas, {
            where: {
