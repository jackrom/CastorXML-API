module.exports = app => {
     ...
     */
    app.get("/resultadosantIfluc", (req, res) => {
        ResultadosAntIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/resultadosantIfluc/:id", (req, res) => {
        ResultadosAntIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/resultadosantIfluc/:id", (req, res) => {
        ResultadosAntIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/resultadosantIfluc", (req, res) => {
        ...
        ResultadosAntIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    ResultadosAntIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    ResultadosAntIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/resultadosantIfluc", (req, res) => {
        ResultadosAntIfluc.update(req.body.cuentas, {
            where: {
