module.exports = app => {
     ...
     */
    app.get("/gastosdeventastxt", (req, res) => {
        GastosdeventasTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/gastosdeventastxt/:id", (req, res) => {
        GastosdeventasTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/gastosdeventastxt/:id", (req, res) => {
        GastosdeventasTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/gastosdeventastxt", (req, res) => {
        ...
        GastosdeventasTxt.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    GastosdeventasTxt.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    GastosdeventasTxt.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/gastosdeventastxt", (req, res) => {
        GastosdeventasTxt.update(req.body.cuentas, {
            where: {
