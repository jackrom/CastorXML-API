module.exports = app => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/ecpIfluc/:id", (req, res) => {
        EcpIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/ecpIfluc/:id", (req, res) => {
        EcpIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/ecpIfluc", (req, res) => {
        EcpIfluc.update(req.body.cuentas, {
            where: {
