...
module.exports = app => {
    const TiposIdentificacion = app.db.models.TiposIdentificacion
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/tiposidentificacion/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                TiposIdentificacion.findById(
                    {
                        id: req.params.id
                    }
                )
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/tiposidentificacion/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                TiposIdentificacion.destroy({where: {id: req.params.id}})
                    .then(res => {
                        res.json(result)
                    })
                    .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/tiposidentificacion", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                TiposIdentificacion.create(req.body)
                    .then(result => {
                        res.json(result)
                    })
                    .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/tiposidentificacion", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                TiposIdentificacion.update(req.query, {where: {id: req.query.id}})
                    .then(result => {
                        console.log(result)
                        res.json(result)
                        //res.sendStatus(204)
