module.exports = app => {
     ...
     */
    app.get("/gastosfinancierostxt", (req, res) => {
        GastosfinancierosTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/gastosfinancierostxt/:id", (req, res) => {
        GastosfinancierosTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/gastosfinancierostxt/:id", (req, res) => {
        GastosfinancierosTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/gastosfinancierostxt", (req, res) => {
        ...
        GastosfinancierosTxt.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    GastosfinancierosTxt.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    GastosfinancierosTxt.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/gastosfinancierostxt", (req, res) => {
        GastosfinancierosTxt.update(req.body.cuentas, {
            where: {
