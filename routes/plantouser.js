module.exports = app => {
     ...
     */
    app.get("/plantosuserfc", (req, res) => {
        PlanToUserfc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/plantosuserfc/:id", (req, res) => {
        PlanToUserfc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/plantosuserfc/:id", (req, res) => {
        PlanToUserfc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/plantosuserfc", (req, res) => {
        console.log('USER LOCATION REQUEST ', req.body.plan)
        PlanToUserfc.create(req.body.plan)
            .then(result => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/plantosuserfc", (req, res) => {
        console.log('UPDATE LOCATIONS REQUEST', req.body)
        PlanToUserfc.update(req.body.obj, {
            where: {
