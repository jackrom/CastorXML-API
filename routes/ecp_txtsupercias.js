module.exports = app => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/ecptxt/:id", (req, res) => {
        EcpTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/ecptxt/:id", (req, res) => {
        EcpTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/ecptxt", (req, res) => {
        EcpTxt.update(req.body.cuentas, {
            where: {
