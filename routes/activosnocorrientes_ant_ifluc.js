module.exports = app => {
     ...
     */
    app.get("/activosnocorrientesantIfluc", (req, res) => {
        ActivosnocorrientesAntIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/activosnocorrientesantIfluc/:id", (req, res) => {
        ActivosnocorrientesAntIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/activosnocorrientesantIfluc/:id", (req, res) => {
        ActivosnocorrientesAntIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/activosnocorrientesantIfluc", (req, res) => {
        ...
        ActivosnocorrientesAntIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    ActivosnocorrientesAntIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    ActivosnocorrientesAntIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/activosnocorrientesantIfluc", (req, res) => {
        ActivosnocorrientesAntIfluc.update(req.body.cuentas, {
            where: {
