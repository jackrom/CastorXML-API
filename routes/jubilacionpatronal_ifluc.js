module.exports = app => {
     ...
     */
    app.get("/jubilacionpatronalifluc", (req, res) => {
        JubilacionPatronalIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/jubilacionpatronalifluc/:id", (req, res) => {
        JubilacionPatronalIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/jubilacionpatronalifluc/:id", (req, res) => {
        JubilacionPatronalIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/jubilacionpatronalifluc", (req, res) => {
        ...
        JubilacionPatronalIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    JubilacionPatronalIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    JubilacionPatronalIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/jubilacionpatronalifluc", (req, res) => {
        JubilacionPatronalIfluc.update(req.body.cuentas, {
            where: {
