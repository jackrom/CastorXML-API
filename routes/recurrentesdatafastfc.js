module.exports = app => {
     ...
     */
    app.get("/recurrentesdatafastfc", (req, res) => {
        RecurrentesdatafastFC.findAll({
            include: [
                {
                    model: UsersFC,
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/recurrentesdatafastfc/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                RecurrentesdatafastFC.findOne({where: {id: req.params.id}})
                    .then(result => res.json(result))
                    .catch(error => {
                        res.status(412).json({msg: error.message});
                    });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/recurrentesdatafastfc/:id", (req, res) => {
        RecurrentesdatafastFC.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/recurrentesdatafastfc", (req, res) => {
        // console.log(req.body)
        RecurrentesdatafastFC.create(req.body.recurrente)
            .then(result => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/recurrentesdatafastfc", (req, res) => {
        // console.log(req.body)
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                let datos = {
                ...
                RecurrentesdatafastFC.update(datos, {
                    where: {
                        id: req.body.obj.id
                    }
                })
