module.exports = app => {
     ...
     */
    app.get("/jubilacionpatronalantifluc", (req, res) => {
        JubilacionPatronalAntIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/jubilacionpatronalantifluc/:id", (req, res) => {
        JubilacionPatronalAntIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/jubilacionpatronalantifluc/:id", (req, res) => {
        JubilacionPatronalAntIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/jubilacionpatronalantifluc", (req, res) => {
        ...
        JubilacionPatronalAntIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    JubilacionPatronalAntIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    JubilacionPatronalAntIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/jubilacionpatronalantifluc", (req, res) => {
        JubilacionPatronalAntIfluc.update(req.body.cuentas, {
            where: {
