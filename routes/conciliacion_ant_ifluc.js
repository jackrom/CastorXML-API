module.exports = app => {
     ...
     */
    app.get("/conciliacionantIfluc", (req, res) => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/conciliacionantIfluc/:id", (req, res) => {
        ConciliacionAntIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/conciliacionantIfluc/:id", (req, res) => {
        ConciliacionAntIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
        ...
        ConciliacionAntIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    ConciliacionAntIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    ConciliacionAntIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/conciliacionantIfluc", (req, res) => {
        ConciliacionAntIfluc.update(req.body.cuentas, {
            where: {
