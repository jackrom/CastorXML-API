module.exports = app => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/aplicacionestouserfc/:id", (req, res) => {
        AplicacionesToUserfc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/aplicacionestouserfc/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                AplicacionesToUserfc.destroy({where: {id: req.params.id}})
                    .then(res => {
                        res.json(result)
                    })
                    .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/aplicacionestouserfc", (req, res) => {
        console.log(req.body.user)
        AplicacionesToUserfc.create(req.body.user)
            .then(result => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/aplicacionestouserfc", (req, res) => {
        // console.log(req.body.user)
        AplicacionesToUserfc.update(req.body.user, {
            where: {
                user_id: req.body.user.user_id,
                aplicacionId: req.body.user.aplicacionId
            }
        })
            .then(result => {
                console.log(result)
