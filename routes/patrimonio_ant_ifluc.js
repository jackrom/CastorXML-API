module.exports = app => {
     ...
     */
    app.get("/patrimonioantIfluc", (req, res) => {
        PatrimonioAntIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/patrimonioantIfluc/:id", (req, res) => {
        PatrimonioAntIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/patrimonioantIfluc/:id", (req, res) => {
        PatrimonioAntIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/patrimonioantIfluc", (req, res) => {
        ...
        PatrimonioAntIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    PatrimonioAntIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    PatrimonioAntIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/patrimonioantIfluc", (req, res) => {
        PatrimonioAntIfluc.update(req.body.cuentas, {
            where: {
