...
module.exports = app => {
    ...
    app.post("/empresastxt/uploadlogo", upload.single('logo'), (req, res, next) => {
        // console.log(req)
    ...
    app.post("/empresastxt/uploadcertificado", upload.single('certificado'), (req, res, next) => {
        const file = req.file
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/empresastxt/byuser", (req, res) => {
        // console.log(req)
        EmpresasTxt.findAll({
            include: [
                {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/empresastxtByUser", (req, res) => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/empresastxt/:id", (req, res) => {
        EmpresasTxt.findOne({
            where: {
                ruc: req.params.id
            }
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/empresastxt/:id", (req, res) => {
        PeriodosTxt.findAll({
            where: {
            ...
            .then(results => {
                if (results && results.length > 0) {
                    results.forEach(obj => {
                        PeriodosTxt.destroy({where: {id: obj.id}})
                    })
                }
                ReportesTxt.findAll({
                    where: {
                ...
                .then(results => {
                    if (results && results.length > 0) {
                        results.forEach(obj => {
                            ReportesTxt.destroy({where: {reporteId: obj.reporteId}})
                        })
                    }
                })
                EmpresasTxt.destroy({where: {id: req.params.id}})
                    .then(result => {
                        res.json(result)
                    })
            })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/empresastxt", (req, res) => {
        // console.log(req.body)
        EmpresasTxt.create(req.body.empresa)
            .then(result => {
                res.json(result)
            })
            .catch(error => {
        ...
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                ...
                Documentos.findAndCountAll({
                    ...
                    .then(result => {
                        if (result.count === 0) {
                            EmpresasTxt.update(empresa, {where: {id: req.body.obj.id}})
                                .then(result => {
                                    console.log(result)
                                    res.json(result)
                                    //res.sendStatus(204)
