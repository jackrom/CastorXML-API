module.exports = app => {
    const RecurrentesdatafastcobrosFC = app.db.models.Recurrentesdatafastcobrosfc
     ...
     */
    app.get("/recurrentesdatafastcobrosfc", (req, res) => {
        RecurrentesdatafastcobrosFC.findAll({
            include: [
                {
                    model: UsersFC,
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/recurrentesdatafastcobrosfc/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                RecurrentesdatafastcobrosFC.findOne({where: {id: req.params.id}})
                    .then(result => res.json(result))
                    .catch(error => {
                        res.status(412).json({msg: error.message});
                    });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/recurrentesdatafastcobrosfc/:id", (req, res) => {
        RecurrentesdatafastcobrosFC.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/recurrentesdatafastcobrosfc", (req, res) => {
        // console.log(req.body)
        RecurrentesdatafastcobrosFC.create(req.body.transaccion)
            .then(result => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/recurrentesdatafastcobrosfc", (req, res) => {
        // console.log(req.body)
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                let datos = {
                ...
                RecurrentesdatafastcobrosFC.update(datos, {
                    where: {
                        id: req.body.obj.id
                    }
                })
