module.exports = app => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/Aplicacionesfc/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Aplicacionesfc.findOne({where: {idAplicacion: req.params.id}})
                    .then(result => res.json(result))
                    .catch(error => {
                        res.status(412).json({msg: error.message});
                    });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/aplicacionesfc/:id", (req, res) => {
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Aplicacionesfc.destroy({where: {idAplicacion: req.params.id}})
                    .then(res => {
                        res.json(result)
                    })
                    .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/aplicacionesfc", (req, res) => {
        // console.log(req.body)
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                Aplicacionesfc.create(req.body)
                    .then(result => {
                        res.json(result)
                    })
                    .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/aplicacionesfc", (req, res) => {
        // console.log(req.body)
        if (req.headers.authorization) {
            if(app.auth.authenticate(req.headers.authorization.replace('Bearer ',''))) {
                let datos = {
                ...
                Aplicacionesfc.update(datos, {
                    where: {
                        idAplicacion: req.body.obj.id
                    }
                })
                    .then(result => {
                        console.log(result)
