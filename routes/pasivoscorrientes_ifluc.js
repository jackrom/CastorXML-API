module.exports = app => {
     ...
     */
    app.get("/pasivoscorrientesIfluc", (req, res) => {
        PasivoscorrientesIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/pasivoscorrientesIfluc/:id", (req, res) => {
        PasivoscorrientesIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/pasivoscorrientesIfluc/:id", (req, res) => {
        PasivoscorrientesIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/pasivoscorrientesIfluc", (req, res) => {
        ...
        PasivoscorrientesIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    PasivoscorrientesIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    PasivoscorrientesIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/pasivoscorrientesIfluc", (req, res) => {
        PasivoscorrientesIfluc.update(req.body.cuentas, {
            where: {
