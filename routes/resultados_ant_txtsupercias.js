module.exports = app => {
     ...
     */
    app.get("/resultadosanttxt", (req, res) => {
        ResultadosAntTxt.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/resultadosanttxt/:id", (req, res) => {
        ResultadosAntTxt.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/resultadosanttxt/:id", (req, res) => {
        ResultadosAntTxt.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/resultadosanttxt", (req, res) => {
        ...
        ResultadosAntTxt.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    ResultadosAntTxt.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    ResultadosAntTxt.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/resultadosanttxt", (req, res) => {
        ResultadosAntTxt.update(req.body.cuentas, {
            where: {
