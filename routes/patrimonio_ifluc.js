module.exports = app => {
     ...
     */
    app.get("/patrimonioIfluc", (req, res) => {
        PatrimonioIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/patrimonioIfluc/:id", (req, res) => {
        PatrimonioIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/patrimonioIfluc/:id", (req, res) => {
        PatrimonioIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/patrimonioIfluc", (req, res) => {
        ...
        PatrimonioIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    PatrimonioIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    PatrimonioIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/patrimonioIfluc", (req, res) => {
        PatrimonioIfluc.update(req.body.cuentas, {
            where: {
