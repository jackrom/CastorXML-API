...
module.exports = app => {
    const Cuponesfc = app.db.models.Cuponesfc
    const Usoscuponesfc = app.db.models.Usoscuponesfc
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/cuponesfc/codigo/:codigo", (req, res) => {
        Cuponesfc.findOne({
            include: [
                {
                    model: Usoscuponesfc,
            ...
            attributes: {
                include: [
                    [
                        Sequelize.literal(`(
                            SELECT COUNT(userId) FROM Usoscuponesfcs WHERE Cuponesfc.codigo = Usoscuponesfcs.cuponCodigo
                        )`),
                        'usuarios'
                    ]
                ]
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/cuponesfc/:id", (req, res) => {
        Cuponesfc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/cuponesfc", (req, res) => {
        // console.log(req.body.cupon)
        Cuponesfc.create(req.body.cupon)
            .then(result => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/cuponesfc", (req, res) => {
        // console.log(req.body.cupon)
        Cuponesfc.update(req.body.cupon, {
            where: {
                id: req.body.cupon.identificacion
            }
        })
            .then(result => {
                console.log(result)
