module.exports = app => {
     ...
     */
    app.get("/gastosfinancierosIfluc", (req, res) => {
        GastosfinancierosIfluc.findAll({
            order: [
                ['id', 'DESC'],
            ]
        })
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/gastosfinancierosIfluc/:id", (req, res) => {
        GastosfinancierosIfluc.findOne({where: {id: req.params.id}})
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/gastosfinancierosIfluc/:id", (req, res) => {
        GastosfinancierosIfluc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/gastosfinancierosIfluc", (req, res) => {
        ...
        GastosfinancierosIfluc.findAll({
            ...
            .then(resultados => {
                if (resultados && resultados.length > 0) {
                    GastosfinancierosIfluc.update(reporte, {
                        where: {
                            id: resultados[0].id
                        }
                    })
                    ...
                    GastosfinancierosIfluc.create(reporte)
                        .then(result => {
                            res.json(result)
                        })
                        .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/gastosfinancierosIfluc", (req, res) => {
        GastosfinancierosIfluc.update(req.body.cuentas, {
            where: {
