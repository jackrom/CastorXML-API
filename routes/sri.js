module.exports = app => {
    const Srifc = app.db.models.Srifc
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/srifc/bycompany", (req, res) => {
        Srifc.findAll({
            where: {
    ...
    app.get("/srifc/byuser", (req, res) => {
        Srifc.findAll({
            ...
            .then(results => {
                ...
                const totalReporteSri = filteredReportes.length
                ...
                res.json({ reportes: filteredReportes, totalPage, totalReporteSri})
            })
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.get("/srifc/:id", (req, res) => {
        Srifc.findById(req.params.id)
            .then(result => res.json(result))
            .catch(error => {
                res.status(412).json({msg: error.message});
            });
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.delete("/srifc/:id", (req, res) => {
        Srifc.destroy({where: {id: req.params.id}})
            .then(res => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.post("/srifc", (req, res) => {
        console.log(req.body)
        Srifc.create(req.body.periodo)
            .then(result => {
                res.json(result)
            })
            .catch(error => {
     ...
     * HTTP/1.1 412 Precondition Failed
     */
    app.put("/srifc", (req, res) => {
        Srifc.update(req.query, {where: {id: req.query.id}})
            .then(result => {
                console.log(result)
                res.json(result)
                //res.sendStatus(204)
